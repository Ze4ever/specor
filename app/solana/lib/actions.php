...
{
    $overrides = store_get_sol_pool_overrides($userId);
    $tokenTargets = store_get_sol_token_targets($userId);
    return [
        'meta' => [
        ...
        'asset_prices' => store_get_sol_asset_prices($userId),
        'dados_uniswap' => store_get_sol_transactions($userId),
        ...
        'pool_order' => store_get_sol_pool_order($userId),
        'token_targets' => $tokenTargets,
        'fee_snapshots' => store_get_sol_fee_snapshots($userId),
        'closed_pools' => store_get_sol_closed_pools($userId),
        'stakes' => store_get_sol_stakes($userId),
    ];
}
...
function normalize_backup_payload(array $payload): array
{
    ...
    if (is_array($stakes)) {
        foreach ($stakes as $row) {
            ...
            if ($token === '' || !is_valid_symbol($token)) {
                continue;
            }
            $normalizedStakes[] = [
                'wallet' => normalize_wallet_label((string) ($row['wallet'] ?? '')),
                ...
                'token' => $token,
                'amount_tokens' => max(0.0, (float) ($row['amount_tokens'] ?? 0.0)),
                'amount_usd' => max(0.0, (float) ($row['amount_usd'] ?? 0.0)),
                'apy' => max(0.0, (float) ($row['apy'] ?? 0.0)),
                'rewards_usd' => max(0.0, (float) ($row['rewards_usd'] ?? 0.0)),
                'start_date' => normalize_datetime_input((string) ($row['start_date'] ?? db_now())),
...
function handle_post_actions(string $importJsonPath): array
{
    ...
    if ($action === 'export_transactions_csv') {
        $rows = store_get_sol_transactions($userId);
        $csvRows = [];
        foreach ($rows as $row) {
            $csvRows[] = [
                (int) ($row['id'] ?? 0),
    ...
    if ($action === 'export_asset_prices_csv') {
        $rows = [];
        foreach (store_get_sol_asset_prices($userId) as $token => $price) {
    ...
    if ($action === 'import_user_data_file') {
        ...
        store_replace_sol_asset_prices($userId, $normalized['asset_prices']);
        store_replace_sol_transactions($userId, $normalized['dados_uniswap']);
        store_replace_sol_pool_overrides($userId, $normalized['pool_overrides']);
        store_replace_sol_pool_order($userId, $normalized['pool_order']);
        store_replace_sol_token_targets($userId, $normalized['token_targets']);
        store_replace_sol_fee_snapshots($userId, $normalized['fee_snapshots']);
        store_replace_sol_closed_pools($userId, $normalized['closed_pools']);
        store_replace_sol_stakes($userId, $normalized['stakes'] ?? []);
    ...
    if ($action === 'add_price') {
        ...
        store_upsert_sol_asset_price($userId, $token, $price);
    ...
    if ($action === 'batch_update_prices') {
        ...
        foreach ($decoded as $token => $price) {
            ...
            store_upsert_sol_asset_price($userId, $tk, (float) $price);
            $updated++;
        }
    ...
    if ($action === 'remove_price') {
        ...
        store_remove_sol_asset_price($userId, $token);
    ...
    if ($action === 'add_tx') {
        ...
        $assetPrices = store_get_sol_asset_prices($userId);
        $dados = store_get_sol_transactions($userId);
        $overrides = store_get_sol_pool_overrides($userId);
        $feeStats = build_fee_stats(store_get_sol_fee_snapshots($userId));
        $latestByPool = store_get_sol_latest_unclaimed_by_pool($userId);
        ...
        store_insert_sol_transaction($userId, [
        ...
        return ['Linha adicionada em Dados_Solana.', 'ok'];
    }
    ...
    if ($action === 'update_tx') {
        ...
        $existing = store_get_sol_transaction_by_id($userId, $txId);
        if ($existing === null) {
        ...
        $ok = store_update_sol_transaction($userId, $txId, [
            'wallet' => $wallet,
            'chain' => $chain,
            'transaction' => $transaction !== '' ? $transaction : 'manual:tx',
    ...
    if ($action === 'remove_tx') {
        ...
        $txRow = store_get_sol_transaction_by_id($userId, $txId);
        if ($txRow === null) {
            return ['Row not found.', 'error'];
        }
        store_delete_sol_transaction($userId, $txId);
        ...
        if ($txAction === 'fees' && $poolId !== '') {
            ...
            if (abs($amount) > 0.0000001) {
                ...
                $targetDate = store_get_sol_latest_fee_snapshot_date($userId, $poolId) ?? date('Y-m-d');
                $newUnclaimed = max(0.0, $baseUnclaimed + $amount);
                store_upsert_sol_fee_snapshot($userId, $poolId, $targetDate, $newUnclaimed);
            }
        }
    ...
    if ($action === 'save_pool_override') {
        ...
        $currentOverrides = store_get_sol_pool_overrides($userId);
        $keepUnclaimed = isset($currentOverrides[$poolId]) ? (float) ($currentOverrides[$poolId]['unclaimed'] ?? 0.0) : 0.0;
        store_upsert_sol_pool_override($userId, $poolId, $current1, $current2, $keepUnclaimed);
    ...
    if ($action === 'save_pool_order') {
        ...
        store_replace_sol_pool_order($userId, array_values(array_unique($normalized)));
    ...
    if ($action === 'save_token_target') {
        ...
        $currentMap = store_get_sol_token_targets($userId);
        ...
        store_upsert_sol_token_target($userId, $token, $targetPct);
    ...
    if ($action === 'save_daily_fees') {
        ...
        foreach ($poolIds as $idx => $poolIdRaw) {
            ...
            store_upsert_sol_fee_snapshot($userId, $poolId, $snapshotDate, $value);
            $saved++;
        }
    ...
    if ($action === 'delete_fee_snapshot') {
        ...
        $deleted = store_delete_sol_fee_snapshot($userId, $poolId, $snapshotDate);
        if (!$deleted) {
    ...
    if ($action === 'delete_fee_snapshot_day') {
        ...
        $allSnaps = store_get_sol_fee_snapshots($userId);
        $deleted = 0;
        foreach ($allSnaps as $snapRow) {
            ...
            if (store_delete_sol_fee_snapshot($userId, $poolId, $snapshotDate)) {
                $deleted++;
            }
        }
        if ($deleted === 0) {
    ...
    if ($action === 'claim_pool_fees') {
        ...
        $latestSnapshotDate = store_get_sol_latest_fee_snapshot_date($userId, $poolId);
        if ($latestSnapshotDate !== null) {
            store_upsert_sol_fee_snapshot($userId, $poolId, $latestSnapshotDate, 0.0);
        }
        store_upsert_sol_fee_snapshot($userId, $poolId, date('Y-m-d'), 0.0);
    ...
    if ($action === 'remove_pool_fees') {
        ...
        $latestClaim = store_get_sol_latest_unreverted_claim_fee_tx($userId, $poolId);
        ...
        store_upsert_sol_fee_snapshot($userId, $poolId, $targetDate, $restoredUnclaimed);
    ...
    if ($action === 'compound_pool') {
        ...
        if (isset($overrides[$poolId])) {
            $ov = $overrides[$poolId];
            store_upsert_sol_pool_override(
                $userId,
                $poolId,
    ...
    if ($action === 'close_pool') {
        ...
        $pool = $snapshotPools[$poolId];
        $closedAt = db_now();
        store_upsert_sol_closed_pool($userId, [
        ...
        store_replace_sol_pool_order(
            $userId,
            array_values(array_filter(store_get_sol_pool_order($userId), static fn($id) => $id !== $poolId))
        );
    ...
    if ($action === 'restore_pool') {
        ...
        $restored = store_delete_sol_closed_pool($userId, $poolId);
        if (!$restored) {
            return ['Pool not found in closed pools.', 'error'];
        }

        $order = store_get_sol_pool_order($userId);
        if (!in_array($poolId, $order, true)) {
            $order[] = $poolId;
            store_replace_sol_pool_order($userId, $order);
        }
    ...
    if ($action === 'add_stake') {
        $token = strtoupper(to_text('stake_token'));
        $wallet = to_text('stake_wallet');
        $validator = to_text('stake_validator');
        $amountTokens = to_float('stake_amount_tokens');
        $amountUsd = to_float('stake_amount_usd');
        $apy = to_float('stake_apy');
        $rewardsUsd = to_float('stake_rewards_usd');
        $startDate = normalize_datetime_input(to_text('stake_start_date'));
        $notes = to_text('stake_notes');

        if (!is_valid_symbol($token)) {
            return ['Token invalido.', 'error'];
        }
        if ($amountTokens < 0 || $amountUsd < 0 || $apy < 0 || $rewardsUsd < 0) {
            return ['Valores invalidos na stake.', 'error'];
        }

        store_insert_sol_stake($userId, [
            'wallet' => $wallet,
            'validator' => $validator,
            'token' => $token,
        ...
        return ['Stake adicionada.', 'ok'];
    }

    if ($action === 'update_stake') {
        $stakeId = (int) ($_POST['stake_id'] ?? 0);
        if ($stakeId <= 0) {
            return ['Stake invalida.', 'error'];
        }
        $token = strtoupper(to_text('stake_token'));
        $wallet = to_text('stake_wallet');
        $validator = to_text('stake_validator');
        ...
        $rewardsUsd = to_float('stake_rewards_usd');
        $startDate = normalize_datetime_input(to_text('stake_start_date'));
        $status = strtolower(to_text('stake_status'));
        $notes = to_text('stake_notes');
        ...
        $ok = store_update_sol_stake($userId, $stakeId, [
            'wallet' => $wallet,
            'validator' => $validator,
            'token' => $token,
            'amount_tokens' => $amountTokens,
        ...
        return [$ok ? 'Stake atualizada.' : 'Stake nao encontrada.', $ok ? 'ok' : 'error'];
    }

    if ($action === 'close_stake') {
        $stakeId = (int) ($_POST['stake_id'] ?? 0);
        if ($stakeId <= 0) {
            return ['Stake invalida.', 'error'];
        }
        $existing = store_get_sol_stake_by_id($userId, $stakeId);
        if ($existing === null) {
            return ['Stake nao encontrada.', 'error'];
        }
        $ok = store_update_sol_stake($userId, $stakeId, array_merge($existing, ['status' => 'closed']));
        return [$ok ? 'Stake encerrada.' : 'Stake nao encontrada.', $ok ? 'ok' : 'error'];
    }

    if ($action === 'remove_stake') {
        $stakeId = (int) ($_POST['stake_id'] ?? 0);
        if ($stakeId <= 0) {
            return ['Stake invalida.', 'error'];
        }
        $ok = store_delete_sol_stake($userId, $stakeId);
        return [$ok ? 'Stake removida.' : 'Stake nao encontrada.', $ok ? 'ok' : 'error'];
    }

    if ($action === 'clear_all') {
        store_clear_sol_all($userId);
        return ['Data cleared (transactions, pools, fees, closed, staking).', 'ok'];
    }
