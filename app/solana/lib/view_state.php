...
function build_view_state(string $importJsonPath): array
{
    ...
    $poolOrderer = store_get_sol_pool_order($userId);
    $closedPools = store_get_sol_closed_pools($userId);
    $closedPoolIds = array_flip(store_get_sol_closed_pool_ids($userId));
    $tokenTargets = store_get_sol_token_targets($userId);
    $feeSnapshots = store_get_sol_fee_snapshots($userId);
    ...
    if ($normalizedOrderer !== $poolOrderer) {
        store_replace_sol_pool_order($userId, $normalizedOrderer);
    }
    ...
    if (!in_array($activeTab, ['dashboard', 'pools', 'create_pool', 'transactions', 'market', 'fees', 'closed', 'staking', 'settings'], true)) {
        $activeTab = 'dashboard';
    }
