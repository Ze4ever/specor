<?php
function store_get_asset_prices(int $userId): array
{
    $stmt = app_pdo()->prepare('SELECT token, price FROM asset_prices WHERE user_id = ? ORDER BY token');
    $stmt->execute([$userId]);
    $out = [];
...
function store_replace_asset_prices(int $userId, array $prices): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
        $del = $pdo->prepare('DELETE FROM asset_prices WHERE user_id = ?');
        $del->execute([$userId]);

        $ins = $pdo->prepare('INSERT INTO asset_prices (user_id, token, price, updated_at) VALUES (?, ?, ?, ?)');
...
function store_upsert_asset_price(int $userId, string $token, float $price): void
{
    $stmt = app_pdo()->prepare(
        'INSERT INTO asset_prices (user_id, token, price, updated_at) VALUES (?, ?, ?, ?)
...
function store_remove_asset_price(int $userId, string $token): void
{
    $stmt = app_pdo()->prepare('DELETE FROM asset_prices WHERE user_id = ? AND token = ?');
    $stmt->execute([$userId, $token]);
}

function store_get_transactions(int $userId): array
{
    $stmt = app_pdo()->prepare(
        'SELECT
...
function store_insert_transaction(int $userId, array $tx): void
{
    $stmt = app_pdo()->prepare(
...
function store_delete_transaction(int $userId, int $txId): void
{
    $stmt = app_pdo()->prepare('DELETE FROM transactions WHERE user_id = ? AND id = ?');
    $stmt->execute([$userId, $txId]);
}

function store_update_transaction(int $userId, int $txId, array $tx): bool
{
    $stmt = app_pdo()->prepare(
        'UPDATE transactions
...
function store_get_transaction_by_id(int $userId, int $txId): ?array
{
    $stmt = app_pdo()->prepare(
        'SELECT
            id, wallet, chain_name, transaction_url, uniswap_url, pool_id, action_name, tx_date,
...
function store_replace_transactions(int $userId, array $rows): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
        ...
        foreach ($rows as $row) {
            store_insert_transaction($userId, $row);
        }
        $pdo->commit();
    } catch (Throwable $e) {
        $pdo->rollBack();
...
function store_get_pool_overrides(int $userId): array
{
    $stmt = app_pdo()->prepare('SELECT pool_id, current_1, current_2, unclaimed, total_usd_override, last_sync_at FROM pool_overrides WHERE user_id = ?');
    $stmt->execute([$userId]);
    $out = [];
    foreach ($stmt->fetchAll() as $row) {
...
function store_upsert_pool_override(int $userId, string $poolId, float $current1, float $current2, float $unclaimed): void
{
    $poolId = normalize_pool_id($poolId);
    $stmt = app_pdo()->prepare(
        'INSERT INTO pool_overrides (user_id, pool_id, current_1, current_2, unclaimed, total_usd_override) VALUES (?, ?, ?, ?, ?, ?)
...
function store_upsert_pool_override_with_total(
    int $userId,
    string $poolId,
    float $current1,
    float $current2,
...
): void
{
    $poolId = normalize_pool_id($poolId);
    $stmt = app_pdo()->prepare(
        'INSERT INTO pool_overrides (user_id, pool_id, current_1, current_2, unclaimed, total_usd_override, last_sync_at) VALUES (?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(user_id, pool_id) DO UPDATE SET
            current_1=excluded.current_1,
            current_2=excluded.current_2,
            unclaimed=excluded.unclaimed,
...
function store_clear_pool_overrides(int $userId): void
{
    app_pdo()->prepare('DELETE FROM pool_overrides WHERE user_id = ?')->execute([$userId]);
}

function store_get_pool_order(int $userId): array
{
    $stmt = app_pdo()->prepare('SELECT pool_id FROM pool_order WHERE user_id = ? ORDER BY ord ASC');
    $stmt->execute([$userId]);
    return array_map(static fn($r) => normalize_pool_id((string) $r['pool_id']), $stmt->fetchAll());
}

function store_replace_pool_order(int $userId, array $order): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
        $pdo->prepare('DELETE FROM pool_order WHERE user_id = ?')->execute([$userId]);
        $ins = $pdo->prepare('INSERT INTO pool_order (user_id, pool_id, ord) VALUES (?, ?, ?)');
...
function store_get_token_targets(int $userId): array
{
    $stmt = app_pdo()->prepare('SELECT token, target_pct FROM token_targets WHERE user_id = ? ORDER BY token ASC');
    $stmt->execute([$userId]);
    $out = [];
    foreach ($stmt->fetchAll() as $row) {
...
function store_upsert_token_target(int $userId, string $token, float $targetPct): void
{
    $cleanToken = strtoupper(trim($token));
    $stmt = app_pdo()->prepare(
        'INSERT INTO token_targets (user_id, token, target_pct, updated_at) VALUES (?, ?, ?, ?)
...
function store_get_fee_snapshots(int $userId): array
{
    $stmt = app_pdo()->prepare(
...
function store_upsert_fee_snapshot(int $userId, string $poolId, string $snapshotDate, float $unclaimedUsd): void
{
    $stmt = app_pdo()->prepare(
        'INSERT INTO fee_snapshots (user_id, pool_id, snapshot_date, unclaimed_usd, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
...
function store_delete_fee_snapshot(int $userId, string $poolId, string $snapshotDate): bool
{
    $stmt = app_pdo()->prepare(
...
function store_get_latest_unclaimed_by_pool(int $userId): array
{
    $stmt = app_pdo()->prepare(
...
function store_get_latest_fee_snapshot_date(int $userId, string $poolId): ?string
{
    $stmt = app_pdo()->prepare(
...
function store_get_latest_unreverted_claim_fee_tx(int $userId, string $poolId): ?array
{
    $stmt = app_pdo()->prepare(
...
function store_upsert_closed_pool(int $userId, array $row): void
{
    $stmt = app_pdo()->prepare(
        'INSERT INTO closed_pools (
            user_id, pool_id, asset_1, asset_2, chain_name, wallet,
            initial_total, total_now, unclaimed, claimed, roi, apr, days_open, hodl_at_close, closed_at
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(user_id, pool_id) DO UPDATE SET
            ...
            hodl_at_close=excluded.hodl_at_close,
    ...
    $stmt->execute([
        ...
        (float) ($row['hodl_at_close'] ?? 0),
        (string) ($row['closed_at'] ?? db_now()),
    ]);
}

function store_get_closed_pools(int $userId): array
{
    $stmt = app_pdo()->prepare(
        'SELECT pool_id, asset_1, asset_2, chain_name, wallet, initial_total, total_now, unclaimed, claimed, roi, apr, days_open, hodl_at_close, closed_at
         FROM closed_pools
         WHERE user_id = ?
    ...
    foreach ($stmt->fetchAll() as $row) {
        $out[] = [
            ...
            'hodl_at_close' => (float) ($row['hodl_at_close'] ?? 0.0),
...
function store_get_closed_pool_ids(int $userId): array
{
    $stmt = app_pdo()->prepare('SELECT pool_id FROM closed_pools WHERE user_id = ?');
    $stmt->execute([$userId]);
    return array_map(static fn($r) => normalize_pool_id((string) $r['pool_id']), $stmt->fetchAll());
}

function store_delete_closed_pool(int $userId, string $poolId): bool
{
    $stmt = app_pdo()->prepare('DELETE FROM closed_pools WHERE user_id = ? AND pool_id = ?');
    $stmt->execute([$userId, normalize_pool_id($poolId)]);
    return $stmt->rowCount() > 0;
}

function store_clear_all(int $userId): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
        $pdo->prepare('DELETE FROM transactions WHERE user_id = ?')->execute([$userId]);
        $pdo->prepare('DELETE FROM pool_overrides WHERE user_id = ?')->execute([$userId]);
        $pdo->prepare('DELETE FROM pool_order WHERE user_id = ?')->execute([$userId]);
        $pdo->prepare('DELETE FROM fee_snapshots WHERE user_id = ?')->execute([$userId]);
        $pdo->prepare('DELETE FROM closed_pools WHERE user_id = ?')->execute([$userId]);
        $pdo->prepare('DELETE FROM token_targets WHERE user_id = ?')->execute([$userId]);
        $pdo->commit();
    } catch (Throwable $e) {
        $pdo->rollBack();
        throw $e;
...
function store_replace_pool_overrides(int $userId, array $rows): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
...
function store_replace_fee_snapshots(int $userId, array $rows): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
...
function store_replace_token_targets(int $userId, array $tokenTargets): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
...
function store_replace_closed_pools(int $userId, array $rows): void
{
    $pdo = app_pdo();
    $pdo->beginTransaction();
    try {
        ...
        foreach ($rows as $row) {
            ...
            $ins->execute([
                ...
                max(0.0, (float) ($row['hodl_at_close'] ?? 0.0)),
