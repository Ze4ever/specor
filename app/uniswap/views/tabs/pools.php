...
<section class="panel table-panel">
  <div class="table-head">
    ...
    <div class="inline-actions">
      <form method="post" id="poolOrderForm" class="inline-form-tight">
        <?= csrf_input() ?>
        <input type="hidden" name="action" value="save_pool_order" />
        <input type="hidden" name="pool_order_json" id="poolOrderJson" value="" />
  ...
  <div class="table-wrap">
    <table class="pools-table">
      <thead><tr><th>Order</th><th>Pool</th><th>Pair / Weight</th><th>ROI & Totals</th><th>Fees & Pace</th><th>Actions</th></tr></thead>
      ...
      <?php else: ?>
        <?php foreach ($poolRows as $i => $pool): ?>
          ...
          <?php
            ...
            $roiTotal = (float) ($pool['roi'] ?? 0.0);
            $roiPct = (float) ($pool['initial_total'] ?? 0.0) > 0.0
              ? ($roiTotal / (float) $pool['initial_total']) * 100.0
              : 0.0;
            $hodlNow = (float) ($pool['hodl_now'] ?? 0.0);
            ...
            <td>
              ...
              <div class="weight-legend-top">
                ...
                <form method="post" class="current-inline" id="<?= h($currentFormId) ?>">
                  ...
                  <label><?= h((string) $pool['asset_1']) ?> - <?= format_money((float) ($pool['atual_1'] ?? 0.0)) ?>
                    <input type="number" step="0.00000001" name="current_1" value="<?= h($current1Input) ?>" />
                  </label>
                  <label><?= h((string) $pool['asset_2']) ?> - <?= format_money((float) ($pool['atual_2'] ?? 0.0)) ?>
            ...
            <td>
              <div class="simple-card">
                <div class="highlight-block">
                  <div class="simple-row highlight-row"><span>ROI Total</span><b class="highlight-value <?= $roiTotal < 0 ? 'error' : 'ok' ?>"><?= format_money($roiTotal) ?></b></div>
                  <div class="simple-row highlight-row"><span>ROI %</span><b class="highlight-value <?= $roiPct < 0 ? 'error' : 'ok' ?>"><?= format_number($roiPct, 2) ?>%</b></div>
                </div>
                <div class="simple-row"><span>Total atual</span><b><?= format_money((float) $pool['total_now']) ?></b></div>
                <div class="simple-row"><span>Fees (U + C)</span><b><?= format_money((float) $feesTotalLp) ?></b></div>
                <div class="simple-row"><span>HODL now</span><b><?= format_money($hodlNow) ?></b></div>
                  ...
                  <div class="detail-sections">
                    <section class="detail-section">
                      ...
                      <div class="detail-grid detail-grid-2">
                        ...
                        <div class="metric-item"><span class="metric-label">Total + Fees</span><span class="metric-value"><?= format_money($portfolioWithFees) ?></span></div>
                        <div class="metric-item"><span class="metric-label">ROI Total</span><span class="metric-value <?= $roiTotal < 0 ? 'error' : 'ok' ?>"><?= format_money($roiTotal) ?></span></div>
                        <div class="metric-item"><span class="metric-label">HODL now</span><span class="metric-value"><?= format_money($hodlNow) ?></span></div>
                      </div>
                    </section>
                    ...
                    <section class="detail-section">
                      ...
                      <div class="detail-grid detail-grid-2">
                        <div class="metric-item"><span class="metric-label">ROI Total</span><span class="metric-value <?= $roiTotal < 0 ? 'error' : 'ok' ?>"><?= format_money($roiTotal) ?></span></div>
                        <div class="metric-item"><span class="metric-label">ROI %</span><span class="metric-value <?= $roiPct < 0 ? 'error' : 'ok' ?>"><?= format_number($roiPct, 2) ?>%</span></div>
                      </div>
                    </section>
                  </div>
